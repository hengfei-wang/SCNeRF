...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, args=None):
    ...
    if spherify:
        ...
        rads = np.percentile(np.abs(tt), 45, 0) #90->45
        c2w_path = c2w
        N_views = 120
        N_rots = 2
        if path_zflat:
#             zloc = np.percentile(tt, 10, 0)[2]
        ...
        # Generate poses for spiral path if args.noise_in_initial_intrinsic:
    ...
    if args.run_without_colmap != "none":
        ...
        if args.run_without_colmap in ["both", "rot"]:
            print("Strating without colmap initialization in rotation")
            base_array = np.array([
                [1., 0., 0.],
                [0., 1., 0.],
                [0., 0., 1.]
        ...
        if args.run_without_colmap in ["both", "trans"]: 
            print("Strating without colmap initialization in translation")
    ...
    poses_update = poses_update.astype(np.float32)
